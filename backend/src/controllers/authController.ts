...
export const register = catchAsync(async (req: Request, res: Response) => {
  ...
  const user = await AuthService.registerUser(
    validation.value as RegisterUserData
  );
...
export const initiatePasswordReset = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const resetToken = await AuthService.initiatePasswordReset(
      (validation.value as { email: string }).email
    );

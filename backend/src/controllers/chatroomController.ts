...
export const getUserChatRooms = catchAsync(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError('User not authenticated', 401);
    }
...
export const getChatRoomById = catchAsync(
  async (req: AuthRequest, res: Response) => {
    ...
    const chatRoom = await ChatService.getChatRoomById(
      roomId,
      req.user._id.toString()
    );
...
export const joinChatRoom = catchAsync(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError('User not authenticated', 401);
    }
    ...
    const chatRoom = await ChatService.joinChatRoom(
      roomId,
      req.user._id.toString()
    );
...
export const leaveChatRoom = catchAsync(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError('User not authenticated', 401);
    }
...
export const updateChatRoom = catchAsync(
  async (req: AuthRequest, res: Response) => {
    if (!req.user) {
      throw new AppError('User not authenticated', 401);
    }
    ...
    const chatRoom = await ChatService.updateChatRoom(
      roomId,
      req.user._id.toString(),
      req.body as Partial<import('@/models').IChatRoom>
    );
